...
public class JasperPdfGenerationServiceImpl implements JasperPdfGenerationService {
    ...
    private Map<String, Object> buildParameters(QuotationDto quotation) {
        ...
        // MRP (list price): pre-computed in QuotationServiceImpl.convertToDto with the entity's
        // margin/tax (so it is correct for every role). Always shown struck-through above the Offer
        // Price (= grand total) — see quotation-grand-summary.jrxml.
        params.put("MRP_FINAL", quotation.getMrpFinal() != null ? quotation.getMrpFinal() : BigDecimal.ZERO);
        ...
        // Kitchen totals data source for the grand summary (MRP + Offer Price). Built whenever there
        // is at least one kitchen so single-kitchen PDFs also show the MRP / Offer Price summary.
        List<QuotationKitchenDto> kitchensList = quotation.getKitchens();
        if (kitchensList != null && !kitchensList.isEmpty()) {
