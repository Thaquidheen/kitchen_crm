 ...
 * UI for assigning and managing staff users for design phases
 */
...
export function StaffAssignment({ designPhase, onUpdate }: StaffAssignmentProps) {
  ...
  const { data: staffList = [], isLoading: staffLoading } = useGetStaffQuery();
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <p className="text-sm text-text-600 mt-1">
              Choose a staff member to assign to this design phase
            </p>
          </div>
      ...
      {formData.staffAssignedId > 0 && (() => {
        const selectedStaff = availableStaff.find(s => s.id === formData.staffAssignedId);
        return selectedStaff ? (
          <Card className="p-6">
            <h3 className="text-lg font-semibold text-text-900 mb-4">Staff Information</h3>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h4 className="font-medium text-text-900 mb-2">Contact Information</h4>
                <div className="space-y-2 text-sm">
                  <p className="text-text-600">Email: {selectedStaff.email}</p>
                  <p className="text-text-600">Phone: {selectedStaff.phoneNumber || 'Not provided'}</p>
                  <p className="text-text-600">Status: {selectedStaff.active ? 'Active' : 'Inactive'}</p>
                </div>
              </div>
            </div>
          </Card>
